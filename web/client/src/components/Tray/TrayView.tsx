...
export function TrayView() {
  ...
  useEffect(() => {
    api.getProxyStatus().then((status: any) => {
      if (status) {
        ...
        setPort(status.port || parseInt(window.location.port) || 3000)
  ...
  const loadData = async () => {
    setIsLoading(true)
    try {
      const [providersData, accountsData, status] = await Promise.all([
        api.getProviders(),
        api.getAccounts(),
        api.getProxyStatus(),
      ])
      ...
      if (status) {
        setProxyRunning(status.isRunning)
        setPort(status.port || window.location.port || 3000)
        setHost(window.location.hostname || '127.0.0.1')
      }
    } catch (error) {
